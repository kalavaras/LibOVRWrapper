...
OVR_PUBLIC_FUNCTION(ovrMatrix4f) ovrMatrix4f_Projection1_3(
    ovrFovPort1_3 fov, float znear, float zfar, unsigned int projectionModFlags)
{
    bool leftHanded     = (projectionModFlags & ovrProjection1_3_LeftHanded) > 0;
    bool flipZ          = (projectionModFlags & ovrProjection1_3_FarLessThanNear) > 0;
    bool farAtInfinity  = (projectionModFlags & ovrProjection1_3_FarClipAtInfinity) > 0;
    bool isOpenGL       = (projectionModFlags & ovrProjection1_3_ClipRangeOpenGL) > 0;
...
OVR_PUBLIC_FUNCTION(ovrTimewarpProjectionDesc1_3) ovrTimewarpProjectionDesc_FromProjection1_3(
    ovrMatrix4f Projection, unsigned int projectionModFlags)
{
    ovrTimewarpProjectionDesc1_3 res;
    ...
    if ( ( projectionModFlags & ovrProjection1_3_ClipRangeOpenGL ) != 0 )
    {
...
OVR_PUBLIC_FUNCTION(ovrMatrix4f) ovrMatrix4f_OrthoSubProjection1_3(
...
OVR_PUBLIC_FUNCTION(void) ovr_CalcEyePoses1_3(ovrPosef1_3 headPose,
    const ovrVector3f hmdToEyeOffset[2],
    ovrPosef1_3 outEyePoses[2])
{
...
OVR_PUBLIC_FUNCTION(void) ovr_GetEyePoses1_3(ovrSession1_3 session, long long frameIndex, ovrBool latencyMarker,
    const ovrVector3f hmdToEyeOffset[2],    
    ovrPosef1_3 outEyePoses[2],
    double* outSensorSampleTime)
{
    double frameTime = ovr_GetPredictedDisplayTime1_3(session, frameIndex);
    ovrTrackingState1_3 trackingState = ovr_GetTrackingState1_3(session, frameTime, latencyMarker);
    ovr_CalcEyePoses1_3(trackingState.HeadPose.ThePose, hmdToEyeOffset, outEyePoses);

    if (outSensorSampleTime != nullptr)
    {
        *outSensorSampleTime = ovr_GetTimeInSeconds1_3();
    }
}

OVR_PUBLIC_FUNCTION(ovrDetectResult1_3) ovr_Detect1_3(int timeoutMilliseconds)
{
    // Initially we assume everything is not running.
    ovrDetectResult1_3 result;
...
OVR_PUBLIC_FUNCTION(void) ovrPosef_FlipHandedness1_3(const ovrPosef1_3* inPose, ovrPosef1_3* outPose)
{
