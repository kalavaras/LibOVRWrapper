...
float GetAccelJumpPosY(BasicVR * pBasicVR, ovrTrackingState * pTrackingState)
{
    ...
    static float yPos = ovr_GetFloat(pBasicVR->HMD, OVR_KEY_EYE_HEIGHT, 0);
    static float yVel = 0;
    ...
    // Hit floor;
    if (yPos < 1.6f) ///ovr_GetFloat(pBasicVR->HMD, OVR_KEY_EYE_HEIGHT, 0))  No longer works
    {
		yPos = 1.6f;/// ovr_GetFloat(pBasicVR->HMD, OVR_KEY_EYE_HEIGHT, 0);
        yVel = 0;
    }
