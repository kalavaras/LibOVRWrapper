...
struct OculusTexture
{
    ...
    OculusTexture(ovrHmd hmd, int sizeW, int sizeH) : SizeW(sizeW), SizeH(sizeH)
    {
...
struct BasicVR
{
    ...
    //------------------------------------------------------
    BasicVR(HINSTANCE hinst, float scaleWindowW = 0.5f, float scaleWindowH = 0.5f,
        float scaleMirrorW = 1.0f, float scaleMirrorH = 1.0f, bool windowed = true) :
        hinst(hinst)
    {
        ...
        ovrResult actualHMD = ovr_Create(&HMD, &luid);
        ...
        DIRECTX.InitWindowAndDevice(hinst, (int)(scaleWindowW * HmdDesc.Resolution.w),
            (int)(scaleWindowH * HmdDesc.Resolution.h),
            L"Oculus Room Tiny (DX11)", reinterpret_cast<LUID*>(&luid), windowed);

        // Start the sensor which informs of the Rift's pose and motion
        result = ovr_ConfigureTracking(HMD, ovrTrackingCap_Orientation | ovrTrackingCap_MagYawCorrection |
            ovrTrackingCap_Position, 0);
        //VALIDATE(((HMD->HmdCaps & ovrHmdCap_DebugDevice) != 0) || OVR_SUCCESS(result), "Failed to configure tracking.");
        ...
        new(&RoomScene) Scene(false);

        // Create camera in-place
        new(&MainCam) Camera(&XMVectorSet(0.0f, 1.6f, +5.0f, 0), &XMQuaternionIdentity());
    ...
    void DistortAndPresent(int numLayersToRender, D3D11_BOX * optionalBoxForMirrorWindow = 0, ovrTexture * texForMirror = NULL, bool mirror = true)
    {
        ...
        ovr_SubmitFrame(HMD, 0, nullptr, layerHeaders, numLayersToRender);
    ...
    //------------------------------------------------------------
    int Release(HINSTANCE /*hinst*/ = nullptr)
    {
        if (mirrorTexture)
        {
