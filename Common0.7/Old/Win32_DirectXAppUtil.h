...
struct DirectX11
{
	...
	static LRESULT CALLBACK WindowProc(_In_ HWND hWnd, _In_ UINT Msg, _In_ WPARAM wParam, _In_ LPARAM lParam)
	{
		...
		switch (Msg)
		{
		    case WM_KEYDOWN:	p->Key[wParam] = true;		break;
		    case WM_KEYUP:		p->Key[wParam] = false;		break;
		    case WM_DESTROY:	p->Running = false;			break;
    ...
    void InitWindowAndDevice(HINSTANCE hinst, int vpW, int vpH, LPCWSTR title, const LUID* pLuid, bool windowed = true)
	{
        hInstance = hinst;
		WinSizeW = vpW;
		WinSizeH = vpH;
		...
		//Adjust window size to account for style, ie margins, header etc, and then create it
		DWORD wsStyle = WS_OVERLAPPEDWINDOW | WS_VISIBLE;
		RECT size = { 0, 0, vpW, vpH };
		AdjustWindowRect(&size, wsStyle, false);
		Window = CreateWindowW(wc.lpszClassName, title, wsStyle, 0, 0, size.right - size.left, size.bottom - size.top, 0, 0, hinst, 0);
		...
		D3D11CreateDevice(Adapter, Adapter ? D3D_DRIVER_TYPE_UNKNOWN : D3D_DRIVER_TYPE_HARDWARE, 0, 0, 0, 0, D3D11_SDK_VERSION, &Device, 0, &Context);
        Adapter->Release();
		...
		Device->CreateRenderTargetView(BackBuffer, NULL, &BackBufferRT);
		...
		Device->QueryInterface(__uuidof(IDXGIDevice1), (void**)&DXGIDevice1);
	...
	void ReleaseWindow(HINSTANCE /*hinst*/ = nullptr)
	{
